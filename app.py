...
def get_all_calculations():
    try:
        ...
        # Debugging: Log data types to ensure 'order-date' conversion
        logging.debug("Data Types after conversion:")
        logging.debug(df.dtypes)
        
        # Check for any NaT values in 'order-date' after conversion and drop them
        if df['order-date'].isnull().any():
            logging.warning("There are NaT values in 'order-date' which will be dropped.")
            df.dropna(subset=['order-date'], inplace=True)
        
        # Debugging: Log the DataFrame after dropping NaT values
        logging.debug("DataFrame after dropping NaT values:")
        logging.debug(df.head())
        
        # Set 'order-date' as the index and resample by week
        df.set_index('order-date', inplace=True)
        
        # Debugging: Ensure 'order-date' is set as index
        logging.debug("Index after setting 'order-date':")
        logging.debug(df.index)
        
        weekly_data = df.resample('W').agg({
            'product-name': 'count',  # Number of products sold in the week
            'cost-price': 'sum',  # Total cost price
            'selling-price': 'sum'  # Total selling price
        })
        
        # Calculate Profit/Loss
        weekly_data['Profit/Loss'] = weekly_data['selling-price'] - weekly_data['cost-price']
        
        # Reset index to get 'order-date' back as a column
        weekly_data.reset_index(inplace=True)
        
        # Rename columns to match the expected output
        weekly_data.rename(columns={
            'order-date': 'Week Start Date',
            'product-name': 'Number of Products Sold',
            'cost-price': 'Cost Price',
            'selling-price': 'Selling Price'
        }, inplace=True)
        
        # Debugging: Log the transformed DataFrame
        logging.debug("Transformed DataFrame:")
        logging.debug(weekly_data.head())
        
        # Reverse the DataFrame
        weekly_data = weekly_data.iloc[::-1]
        
        # Convert the DataFrame to a list of dictionaries
        sessions = weekly_data.to_dict('records')
...
def calculate():
    if request.method == 'POST':
        session.clear()
        new_calculate_id = create_calculate()  # Create a new calculate session
        session['calculate_id'] = new_calculate_id
        return redirect(url_for('calculate', calculate_id=new_calculate_id))
 
    existing_sessions = get_all_calculations()
    
    # Debugging: Log existing sessions
    logging.debug("Existing Sessions:")
    logging.debug(existing_sessions)
    
    return render_template('calculate.html', existing_sessions=existing_sessions)

@app.route('/chart-data')
def chart_data():
    existing_sessions = get_all_calculations()
    
    weeks = [session['Week Start Date'].strftime('%Y-%m-%d') for session in existing_sessions]
    profit_loss = [session['Profit/Loss'] for session in existing_sessions]
...
def create_calculate():
    try:
        sessions = get_all_calculations()
        
        if not sessions:
            return []
        
        first_date = sessions[-1]['Week Start Date']
        first_date = pd.to_datetime(first_date)
        today = pd.to_datetime('today')
        
        df = pd.read_csv('sales.csv')
        df['order-date'] = pd.to_datetime(df['order-date'], format='%d-%m-%Y', errors='coerce')
        df = df[(df['order-date'] >= first_date) & (df['order-date'] <= today)]
        
        result = df.groupby(pd.Grouper(key='order-date', freq='W')).agg({
            'product-name': 'count',
            'cost-price': 'sum',
            'selling-price': 'sum'
        }).reset_index()
        
        result['Profit/Loss'] = result['selling-price'] - result['cost-price']
